...
2. **DeepSeek API** (`RECOMMEND_PROVIDER=deepseek_api`): Sends query to DeepSeek V4 API, injecting user profile summary and category intent keywords into the prompt via `AGENT_*` parameters. DeepSeek V4 generates the AI-powered recommendation response.
